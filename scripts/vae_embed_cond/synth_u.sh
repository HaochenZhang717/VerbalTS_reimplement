...
do
  ...
  do
    ...
    export WANDB_NAME="vae_embed_cond_synth_u_cosine-lr${LR}_bs${BS}-L${LAYERS}C${CHANNELS}H${NHEADS}D${DIFFUSION_EMBEDDING_DIM}-dropout0.1"

    CUDA_VISIBLE_DEVICES=2 python run_qwen.py \
        --cond_modal vae_embed \
        --training_stage finetune \
        --save_folder ./sweep/vae_embed_cond/lr_${LR}_bs_${BS}-L${LAYERS}C${CHANNELS}H${NHEADS}D${DIFFUSION_EMBEDDING_DIM}-dropout0.1 \
